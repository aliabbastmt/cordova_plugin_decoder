...
/*package-private*/ class SoniTalkPermissionManager implements PermissionLevelDialogActivity.PermissionLevelDialogListener, PermissionRequestDialogActivity.PermissionRequestDialogListener{
    ...
    /*package-private*/ static final String PERMISSION_SONITALK_L0 = "com.churchbase.ismchurch.permission_all_ultrasonic_communication";
    ...
    public void onGrantClick(DialogFragment dialog) {
        ...
        String resource_text = activity.getString(activity.getResources().getIdentifier("context_preference_key", "string", activity.getPackageName()));

        SharedPreferences sp = activity.getSharedPreferences(resource_text, Context.MODE_PRIVATE);
        resource_text = activity.getString(activity.getResources().getIdentifier("bundleRequestCode_key", "string", activity.getPackageName()));
        sp.edit().remove(resource_text).apply();
        synchronized (SoniTalkPermissionManager.class) {
            ...
            ed.putBoolean(activity.getString(activity.getResources().getIdentifier("currentRequestGranted_key", "string", activity.getPackageName())), currentRequestGranted);
            ed.putBoolean(activity.getString(activity.getResources().getIdentifier("currentRequestAnswered_key", "string", activity.getPackageName())), currentRequestGranted);
            ed.apply();
    ...
    public void onDenyClick(DialogFragment dialog) {
        ...
        String resource_text = activity.getString(activity.getResources().getIdentifier("context_preference_key", "string", activity.getPackageName()));
        SharedPreferences sp = activity.getSharedPreferences(resource_text, Context.MODE_PRIVATE);
        resource_text = dialog.requireContext().getString(activity.getResources().getIdentifier("bundleRequestCode_key", "string", activity.getPackageName()));
        sp.edit().remove(resource_text).apply();
        synchronized (SoniTalkPermissionManager.class) {
            currentRequestGranted = false;
            currentRequestAnswered = true;
    ...
    public void onChangeSettingsClick(DialogFragment dialog) {
        ...
        synchronized (SoniTalkPermissionManager.class) {
            ...
            resource_text = activity.getString(activity.getResources().getIdentifier("permission_level_key", "string", activity.getPackageName()));
            ed.putInt(resource_text, -1);
            ed.apply();
    ...
    public void onL0Click(DialogFragment dialog) {
        ...
        resource_text = activity.getString(activity.getResources().getIdentifier("permission_level_key", "string", activity.getPackageName()));
        sp.edit().putInt(resource_text, permissionLevel.get()).apply();
        ...
        if (hasPermissions(activity, PERMISSION_SONITALK_L0)) {
            ...
            synchronized (PermissionLevelDialogActivity.PermissionLevelDialogFragment.class) {
                SharedPreferences.Editor ed = sp.edit();
                resource_text = activity.getString(activity.getResources().getIdentifier("permissionLevelWasSet_key", "string", activity.getPackageName()));
                ed.putBoolean(resource_text, true);
                resource_text = activity.getString(activity.getResources().getIdentifier("permissionLevelWasAnswered_key", "string", activity.getPackageName()));
                ed.putBoolean(resource_text, true);
                ed.apply();
    ...
    public void onDeclineClick(DialogFragment dialog) {
        ...
        synchronized (PermissionLevelDialogActivity.PermissionLevelDialogFragment.class) {
            SharedPreferences.Editor ed = sp.edit();
            resource_text = activity.getString(activity.getResources().getIdentifier("permissionLevelWasSet_key", "string", activity.getPackageName()));
            ed.putBoolean(resource_text, false);
            resource_text = activity.getString(activity.getResources().getIdentifier("permissionLevelWasAnswered_key", "string", activity.getPackageName()));
            ed.putBoolean(resource_text, true);
            ed.apply();
    ...
    /*package-private*/boolean checkPermissionLevelIsSet(Context context) {
        ...
        String resource_text = context.getString(context.getResources().getIdentifier("context_preference_key", "string", context.getPackageName()));

        SharedPreferences sp = context.getSharedPreferences(resource_text, Context.MODE_PRIVATE);
        if (hasPermissions(context, PERMISSION_SONITALK_L0)) {
            ...
            permissionLevel.set(-1);
            SharedPreferences.Editor ed = sp.edit();
            resource_text = context.getString(context.getResources().getIdentifier("permission_level_key", "string", context.getPackageName()));
            ed.putInt(resource_text, permissionLevel.get());
            ed.apply();
            ...
            resource_text = context.getString(context.getResources().getIdentifier("permission_level_key", "string", context.getPackageName()));
            permissionLevel.set(sp.getInt(resource_text, -1));
            ...
            if (permissionLevel.get() == -1 || permissionLevel.get() == 0) {
                ...
                SharedPreferences.Editor ed = sp.edit();
                resource_text = context.getString(context.getResources().getIdentifier("permissionLevelWasSet_key", "string", context.getPackageName()));
                ed.putBoolean(resource_text, permissionLevelWasSet);
                resource_text = context.getString(context.getResources().getIdentifier("permissionLevelWasAnswered_key", "string", context.getPackageName()));
                ed.putBoolean(resource_text, permissionLevelWasAnswered);
                ed.apply();
                ...
                synchronized (PermissionLevelDialogActivity.PermissionLevelDialogFragment.class) {
                    ...
                    try {
                        while (!permissionLevelWasAnswered) {
                            ...
                            resource_text = context.getString(context.getResources().getIdentifier("permissionLevelWasSet_key", "string", context.getPackageName()));
                            permissionLevelWasSet = sp.getBoolean(resource_text, permissionLevelWasSet);
                            resource_text = context.getString(context.getResources().getIdentifier("permissionLevelWasAnswered_key", "string", context.getPackageName()));
                            permissionLevelWasAnswered = sp.getBoolean(resource_text, permissionLevelWasAnswered);
                            resource_text = context.getString(context.getResources().getIdentifier("permission_level_key", "string", context.getPackageName()));
                            permissionLevel.set(sp.getInt(resource_text, -1));
                        }
    ...
    /*package-private*/boolean checkSelfPermission(Context context, int requestCode) {//}, String permission) {
        ...
        SharedPreferences sp = context.getSharedPreferences(resource_text, Context.MODE_PRIVATE);
        SharedPreferences.Editor ed = sp.edit();
        resource_text = context.getString(context.getResources().getIdentifier("currentRequestAction_key", "string", context.getPackageName()));
        ed.putInt(resource_text, requestCode);
        ed.apply();
    ...
    private boolean checkPermissionGranted(Context context) {
        ...
        if (permissionLevel.get() == 0 || permissionLevel.get() == -1) { // Note: We should not store 0
            if (hasPermissions(context, PERMISSION_SONITALK_L0)) {
                ...
                resource_text = context.getString(context.getResources().getIdentifier("bundleRequestCode_key", "string", context.getPackageName()));
                ed.remove(resource_text).apply();
                return true;
            }
            else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (sessionPermissionGranted) { //No need to get it from sharedpref as it is based on a value that was in Sharedpref.
                ...
                synchronized (SoniTalkPermissionManager.class) {
                    currentRequestGranted = false;
                    currentRequestAnswered = false;
                    resource_text = context.getString(context.getResources().getIdentifier("currentRequestGranted_key", "string", context.getPackageName()));
                    ed.putBoolean(resource_text, currentRequestGranted);
                    resource_text = context.getString(context.getResources().getIdentifier("currentRequestAnswered_key", "string", context.getPackageName()));
                    ed.putBoolean(resource_text, currentRequestAnswered);
                    ed.apply();
                    ...
                    try {
                        while (!currentRequestAnswered) {
                            ...
                            currentRequestGranted = sp.getBoolean(resource_text, currentRequestGranted);
                            resource_text = context.getString(context.getResources().getIdentifier("currentRequestAnswered_key", "string", context.getPackageName()));
                            currentRequestAnswered = sp.getBoolean(resource_text, currentRequestAnswered);
                        }
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    ...
                    if (sp.getInt(resource_text, -1) == -1) {
    ...
    /*package-private*/ void sendJobFinished(Context context, int requestCode){
        ...
        String resource_text = context.getString(context.getResources().getIdentifier("bundleRequestCode_key", "string", context.getPackageName()));

        resultData.putInt(resource_text, requestCode);
    ...
    private int getCurrentRequestCode(Context context){
        ...
        SharedPreferences sp = context.getSharedPreferences(resource_text, Context.MODE_PRIVATE);
        resource_text = context.getString(context.getResources().getIdentifier("currentRequestAction_key", "string", context.getPackageName()));
        int requestCode = sp.getInt(resource_text, 0);
        return requestCode;
    }
