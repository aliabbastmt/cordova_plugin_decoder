...
public class PermissionLevelDialogActivity extends AppCompatActivity {
    ...
    public static class PermissionLevelDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            String resource_text = getActivity().getString(getActivity().getResources().getIdentifier("permission_level_question", "string", getActivity().getPackageName()));

            String permissionLevelQuestion = String.format(resource_text, app_name);
            AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());

//            int permission_level_texts = getActivity().getResources().getIdentifier("context_preference_key", "array", getActivity().getPackageName());

            ArrayList<String> list = new ArrayList<>();
            list.add("Low - Allow always");
            list.add("Moderate - Allow until next start of app");
            list.add("Strict - Ask on each communication request");
            list.add("Decline");
                ...
                if (SoniTalkPermissionManager.PERMISSION_SONITALK_L0.equals(permissions[0])) {
                    if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                        ...
                        synchronized (PermissionLevelDialogFragment.class) {
                            String resource_text = getApplication().getString(getApplication().getResources().getIdentifier("context_preference_key", "string", getApplication().getPackageName()));

                            SharedPreferences sp = this.getSharedPreferences(resource_text, Context.MODE_PRIVATE);
                            SharedPreferences.Editor ed = sp.edit();
                            resource_text = getApplication().getString(getApplication().getResources().getIdentifier("permissionLevelWasSet_key", "string", getApplication().getPackageName()));
                            ed.putBoolean(resource_text, true);
                            resource_text = getApplication().getString(getApplication().getResources().getIdentifier("permissionLevelWasAnswered_key", "string", getApplication().getPackageName()));
                            ed.putBoolean(resource_text, true);
                            ed.apply();
