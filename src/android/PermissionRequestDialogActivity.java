...
public class PermissionRequestDialogActivity extends AppCompatActivity {
    ...
    /*package-private*/static final String EXTRA_PERMISSION_REQUEST_LISTENER = "com.churchbase.ismchurch.EXTRA_PERMISSION_REQUEST_LISTENER";
    /*package-private*/static final String EXTRA_PERMISSION_LEVEL_CODE = "com.churchbase.ismchurch.EXTRA_PERMISSION_LEVEL_CODE";
    ...
    public static class PermissionRequestDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            String resource_text = currentActivity.getString(currentActivity.getResources().getIdentifier("permission_request_title", "string", currentActivity.getPackageName()));

            builder.setTitle(resource_text);
            String permissionRequestQuestion = null;
            if(permission_level_code==SoniTalkPermissionManager.PERMISSION_LEVEL_1_CODE) {
                resource_text = currentActivity.getString(currentActivity.getResources().getIdentifier("permission_request_l1_question", "string", currentActivity.getPackageName()));
                permissionRequestQuestion = String.format(resource_text, app_name);
            }else if(permission_level_code==SoniTalkPermissionManager.PERMISSION_LEVEL_2_CODE){
                resource_text = currentActivity.getString(currentActivity.getResources().getIdentifier("permission_request_l2_question", "string", currentActivity.getPackageName()));
                permissionRequestQuestion = String.format(resource_text, app_name);

            }
            String btn_pos = currentActivity.getString(currentActivity.getResources().getIdentifier("permission_request_positive_text", "string", currentActivity.getPackageName()));
            String btn_neg = currentActivity.getString(currentActivity.getResources().getIdentifier("permission_request_negative_text", "string", currentActivity.getPackageName()));
            String btn_neu = currentActivity.getString(currentActivity.getResources().getIdentifier("permission_request_change_text", "string", currentActivity.getPackageName()));

            builder.setMessage(permissionRequestQuestion) // Retrieve sending / receiving action type via Intent extra ?
                    .setPositiveButton(btn_pos, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int id) {
                            //Log.d("permission request", "Positive");
                        listener.onGrantClick(PermissionRequestDialogFragment.this);
                        }
                    })
                    .setNegativeButton(btn_neg, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int id) {
                            //Log.d("permission request", "Negative");
                            dialog.cancel(); // Will call the deny callback
                        }
                    })
                    .setNeutralButton(btn_neu, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int id) {
